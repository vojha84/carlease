...
var logger = require(__dirname+'/src/logging/logging').initialize('CarLease App', 'local', configFile.config.log4js);
...
let constants = require(__dirname+'/src/constants/constants.js');
let datastore = require(__dirname+'/src/database/datastore.js');
let modelFactory = require(__dirname+'/src/database/model_factory.js');
...
function customSetup(){
    return new Promise(function(resolve, reject){
        //Setup datastore
        var cloudantConfig = configFile.config.cloudantNoSQLDB;
       var result = datastore.initSync(cloudantConfig);
       if(result.statusCode != constants.SUCCESS){
        console.error('Could not initialize datastore %j', result);
       ...
       var cloudantKvStore;
       modelFactory.getModel(constants.MODEL_KEYVALUESTORE, configFile.config.databases[constants.CARLEASE_APP_MASTER_DB])
       .then(function(resp){
        cloudantKvStore = resp.body;
        cloudantSetupDone = true;
        console.log(cloudantKvStore);
        chain.setKeyValStore(cloudantKvStore);
